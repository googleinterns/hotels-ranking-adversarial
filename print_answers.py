def _print_query(model_builder):
  ...
  """
  query_tensor = model_builder.query_features_evaluated #a sparse tensor containing a tensor of values and a tensor of indices corresponding to filled values
  values = query_tensor.values #list of words of all questions
...
def _create_answer_array(model_builder):
  ...
  """
  doc_tensor = model_builder.answer_features_evaluated #a sparse tensor containing a tensor of values and a tensor of indices corresponding to values
  indices = doc_tensor.indices
  values = doc_tensor.values #list of words to all answers
...
def _create_ranked_answers_array(model_builder, ranking_array):
  """Combines an array of ranks with an array of features to create an array relating each answer with its associated rank. Used for printing answers.
  Args:
    model_builder: The model in use.
  ...
  ranked_answers.sort(key=lambda x: x[1], reverse = True) #answers with the highest score should come first 
...
def print_settings(model_builder):
  '''
  Prints basic information about the perturbation, including the question, answer and amount of perturbation.
  ...
  print("Answer: ", _create_answer_array(model_builder)[model_builder.answer_number])
