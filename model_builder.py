...
class ModelBuilder:
  ...
  def __init__(self):
    ...
    self.random_noise = False #flag to determine if using random noise
    self.first_eval = True #flag to determine if this is first evaluation (in which case we save tensor values)
    ...
    self.embedded_features_evaluated = [[0.0]*constants._FULL_EMBEDDING]*constants._LIST_SIZE

    #Array containing values for weights in first dense layer and gradient of logits wrt to those weights. Used for FGSM calculation.
    self.grad_variable_pair_tensor = None
    self.grad_variable_pair_evaluated = [[[0]*int(constants._HIDDEN_LAYER_DIMS[0])]*constants._FULL_EMBEDDING]*2

    #Embedding values post batch normalization. Used for FGSM calculation.
    self.normalized_features_evaluated = [[0.0]*constants._FULL_EMBEDDING]*constants._LIST_SIZE
  ...
  def context_feature_columns(self):
    ...
    sparse_column = tf.feature_column.categorical_column_with_vocabulary_file(
        key="query_tokens",
        vocabulary_file=constants._VOCAB_PATH)
  ...
  def input_fn(self, path, num_epochs=None):
    ...
    self.perturb_on = tf.constant(False) #ensures we don't add perturbation during training
  ...
  def make_score_fn(self):
    ...
    def _score_fn(context_features, group_features, mode, params, config):
      ...
      if self.first_eval == False: #we only want to save embedding on first predict run
        input_layer = tf.convert_to_tensor(self.embedded_features_evaluated)
      
      input_layer = tf.cond(self.perturb_on, lambda: self._perturbed_layer(), lambda: self._nonperturbed_layer(input_layer)) #perturb input if indicated
      ...
      cur_layer = tf.compat.v1.layers.batch_normalization(
        ...
        momentum=0.99)
      self.normalized_features = cur_layer #used for FGSM calculation
      
      for i, layer_width in enumerate(int(d) for d in constants._HIDDEN_LAYER_DIMS):
      ...
      logits = tf.compat.v1.layers.dense(cur_layer, units=constants._GROUP_SIZE)
  ...
  def _perturbed_layer(self):
    ...
    noise = get_perturbed_input(self, self.answer_number, self.perturb_amount)
  ...
  def train_and_eval_fn(self, model_fn):
    ...
    train_input_fn = lambda: self.input_fn(constants._TRAIN_DATA_PATH)
    eval_input_fn = lambda: self.input_fn(constants._TEST_DATA_PATH, num_epochs=1)
  ...
  def run_training(self):
    ...
    _LOSS = tfr.losses.RankingLossKey.APPROX_NDCG_LOSS   # Define a loss function. To find a complete list of available loss functions or to learn how to add your own custom function please refer to the tensorflow_ranking.losses module.
  ...
  def predict_input_fn(self, path):
    ...
    context_feature_spec = tf.feature_column.make_parse_example_spec(
          self.context_feature_columns().values())
    label_column = tf.feature_column.numeric_column(
          constants._LABEL_FEATURE, dtype=tf.int64, default_value=constants._PADDING_LABEL)
    example_feature_spec = tf.feature_column.make_parse_example_spec(
          list(self.example_feature_columns().values()) + [label_column])
    dataset = tfr.data.build_ranking_dataset(
          file_pattern=path,
  ...
  def custom_predict(self, perturb, ranker, input_fn,
    ...
    if not checkpoint_path:
      checkpoint_path = checkpoint_management.latest_checkpoint(ranker._model_dir)
    if not checkpoint_path:
    ...
    with tf.Graph().as_default() as g:
      ...
      features, input_hooks = ranker._get_features_from_input_fn(input_fn, ModeKeys.PREDICT)
      estimator_spec = ranker._call_model_fn(features, None, ModeKeys.PREDICT, ranker.config)
      ...
      with training.MonitoredSession(
        ...
        while not mon_sess.should_stop():
          ...
          if self.first_eval: #save values for tensors during first nonperturbed evaluation to be used in next execution
